...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const properties = await select('properties', 'select=id,address,address_key,owner_approval_limit&active=eq.true');
    const normalized = normalizeAddress(extractedAddress);
    const property = properties.find(p => p.address_key === normalized) || null;

    let tenant = null;
    if (property) {
      const tenants = await select('tenants', `select=id,name,email,property_id&property_id=eq.${property.id}&email=eq.${q(from)}&active=eq.true`);
    ...
    const contractors = await select('contractors', `select=id,name,email,trade,response_sla_minutes&trade=eq.${q(route.trade)}&active=eq.true&order=priority_rank.asc&limit=1`);
